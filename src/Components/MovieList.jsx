...
function MovieList({ genreId, index_ }) {
  ...
  useEffect(() => {
    handleGetMovieByGenreId();
  }, []);

  const handleGetMovieByGenreId = async () => {
    const res = await GlobalApi.getMovieByGenreId(genreId);

    console.log(res, 'This is genre');
  ...
  return (
    <div className="relative">
      ...
      <div
        ref={elementRef}
        className="flex overflow-x-auto gap-8  no-scrollbar scroll-smooth pt-4 px-3 pb-4"
      >
        {movieList.map((item) => (
          <>
            {index_ % 3 == 0 ? (
